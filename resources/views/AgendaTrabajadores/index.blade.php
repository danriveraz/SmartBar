      					...
      					<div id="inicio">
							<div class="heading">
								...
								<div class="col-xs-2 col-md-3 text-center">
								  <label>
									  <input class="jornada" type="radio" value="1" name="entities[0][0]">
									  <span id="dia"></span>
								  </label><label>Dia</label>
								</div>
								<div class="col-xs-2 col-md-3 text-center">
								  <label>
									  <input class="jornada" type="radio" value="2" name="entities[0][0]">
									  <span id="noche"></span>
						...
						<div class="widget-container fluid-height">
								...
								<div class="widget-content padded">
									<div class="profile-info clearfix">
										<label class="pull-left">
											<input type="radio" onclick="actualizar({{$numeracion[$key]}},{{$empleado->salario}})" value="{{$empleado->id}}" name="entities[1][0]">
							...
							<div class="col-md-6 text-center">
								...
								<div class="widget-content padded">
									<div class="profile-info clearfix">
										<label class="pull-left">
											<input type="radio" onclick="actualizar({{$numeracion[$numEmpleadosIzq+$key]}},{{$empleado->salario}})" value="{{$empleado->id}}" name="entities[1][0]">
											<span id="usuario"></span>
										</label>
										<img width="70" height="70" class="avatar pull-left" src="images\avatar-male.jpg">
										<div class="profile-details">
...
<script>
	    ...
	    $('#calendar').fullCalendar({
	    	...
	    	viewRender: function(view, element) {
			    ...
			    for (var i = 0; i < turnos.length; i++) {
			    	numeroDias = turnosUsuario(turnos[i], ultimoDia.getDate(), mes, anio);
	...
	function actualizar(id, salario){
	...
	function turnosUsuario(turnos, numeroDias, mes, anio){
		var diasTrabajo = 0;
		for (var i = 1; i <= numeroDias; i++) {
			...
			if(turno == 1){
				diasTrabajo++;
				inicio = moment(deit).format('YYYY-MM-DD')+"T08:00:00";
				fin = moment(deit).format('YYYY-MM-DD')+"T17:00:00";
				myevent = {title: titulo+"",start: inicio, end: fin, color: colorPalette[color-1], allDay: false};
				$('#calendar').fullCalendar( 'renderEvent', myevent);
			}else if(turno == 2){
				diasTrabajo++;
				inicio = moment(deit).format('YYYY-MM-DD')+"T18:00:00";
				fin = moment(deit).format('YYYY-MM-DD')+"T23:00:00";
				myevent = {title: titulo+"",start: inicio, end: fin, color: colorPalette[color-1], allDay: false};
