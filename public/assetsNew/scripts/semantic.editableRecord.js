(function($) {
    ...
    function appendNewButton(editableRecord){
        ...
        editableRecord.newButton.css('float', 'right').insertAfter(editableRecord).on('click.editableRecord', function(){
            newButtonClicked(editableRecord);
        });
    }

    function appendTableHeadForDeleteButton(editableRecord){
        editableRecord.find('thead tr').append('<th name="action"></th>')
    }
